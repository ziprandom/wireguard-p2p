...
async fn create_new_lo_socket(
    ...
    let f = lo_sock2
        .into_stream()
        .map_ok(move |(buf, _)| (buf, inet_peer))
        .forward(inet_tx);
    task::spawn(f);
...
async fn lookup_peers(
    ...
    for (pubkey, _publish_dht_key, lookup_key, key) in remote_peer_keys {
        ...
        let mut rx = rx.filter_map(move |value| {
            future::ready(decrypt(&key, value).ok().and_then(deserialize))
        });
        ...
        task::spawn(async move {
            ...
            while let Some((time, peer_addr)) = rx.next().await {
                ...
                if time > max_time {
                    ...
                    if let Err(send_err) = tx.send((peer_addr, ttx)).await {
                        warn!("error after receiveing new peer_addr: {:?}", send_err);
                        continue;
                    };
                    ...
                    if let Some(mut wg_peer) = wg.get_peer(&pubkey) {
                        ...
                        if let Err(e) = wg.apply() {
                            error!(
                                "Error while setting endpoint {} to {} ({}): {}",
                                pubkey.as_b64(),
                                lo_addr,
                                peer_addr,
                                e
...
async fn publish_peers(
    ...
    loop {
        ...
        let connectivity = match connectivity {
            ...
            Err(error) => {
                warn!("didn't get connectivity info from stun server: {:?}", error);
                continue;
            }
        };
...
async fn handle_device(
    ...
    let (new_dht_peer_tx, new_dht_peer_rx) = bounded(1024);

    let wg_peer = (Ipv4Addr::LOCALHOST, wg_listen_port).into();
    let wg = Arc::new(wg);

    task::spawn(lookup_peers(
        dht.clone(),
        wg.clone(),
        new_dht_peer_tx,
        remote_peer_keys.clone(),
    ));
    task::spawn(publish_peers(
        stun_server,
        inet_tx.clone(),
        inet_rx2,
        wg,
    ...
    loop {
        ...
        match f1.race(f2).await {
            // TODO: order?
            Either::NewDhtPeer(Err(dht_err)) => {
                warn!("dht connection received error: {:?}", dht_err);
                break Ok(());
            }
            Either::InetPacket(None) => {
                panic!("inet closed");
            }
            Either::NewDhtPeer(Ok((inet_peer, tx))) => {
                let create = create_new_lo_socket(inet_tx.clone(), wg_peer, inet_peer);
                let lo_sock = map
                    .entry(inet_peer)
                    .or_try_insert_with_async(create)
                    .await?;

                if tx.send(lo_sock.local_addr()?).is_err() {
                    error!(
                        "{} Send new lo addr failed: receiver dropped.",
                        Colour::Red.paint("HDL")
                    );
                }
            }
            Either::InetPacket(Some((buf, inet_peer))) => {
                ...
                match res {
                    Err(e) => error!(
                        "{} Failed to open lo socket for {}: {}. Dropping {} bytes.",
                        Colour::Red.paint("HDL"),
                        wg_peer,
                        e,
                        buf.len()
...
async fn main() -> io::Result<()> {
    sudo::escalate_if_needed().map_err(|e| {
        io::Error::new(io::ErrorKind::Other, format!("Failed to become root {}", e))
    })?;
    ...
    sodiumoxide::init()
        .map_err(|_| io::Error::new(io::ErrorKind::Other, "Failed to initialize sodiumoxide"))?;
    ...
    let err = || {
        io::Error::new(
            ...
            "Wireguard device has no listen port",
        )
    };
    ...
    let stun_server = stun_server
        .next()
        .ok_or_else(|| io::Error::from_raw_os_error(libc::EDESTADDRREQ))?;

    let secret_key = wg.secret_key();
    let secret_key = secret_key.unwrap_or_else(|| panic!("Device {} has no private key", &ifname));
    let our_pkey = secret_key.public_key();

    let remote_peer_keys = wg
        .peers()
        .iter()
        .filter_map(|p| {
